...
OUTPUT_DIR=tiemposKruskalCompression
...
for n in $(seq $MIN_N 1 $MAX_N); do
    ...
    for intento in $(seq 1 $TRIES_COUNT);do
        kruskalCompressionTime=$(./generarGrafoConexoDenso.py $n | ../main kruskalCompression)
        echo "$n,$kruskalCompressionTime" >> $OUTPUT_FILE
    done
done
